...
song_duration.oninput = function() {
  const percentage = song_duration.value; // Get the current value of the slider
  // Calculate the marquee scroll position based on the slider value
  marquee.scrollTop = (percentage * marquee.scrollHeight) / 100; // Move the marquee based on slider value
};

audio.addEventListener('timeupdate', () => {
  const percentage = (audio.currentTime / audio.duration) * 100; // Get current time as a percentage
  marquee.scrollTop = (percentage * marquee.scrollHeight) / 100; // Move the marquee up based on the song's progress
});
